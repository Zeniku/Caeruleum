...
public class CaeDraw {
  ...
  public static void spike(float x,float y, Color colorFrom, Color colorTo, float inOut, float size, float rotation) {
	...
	public static void spike(float x, float y, int spikes, float size, float length, float rotation){
    float step = 360f / spikes;
    for(int i = 0; i < spikes; i++){
      Drawf.tri(x, y, size, length, i * step + rotation);
    };
  }
